...
class ccw_widget extends WP_Widget {
	...
	function __construct() {
		...
		__('Woo Events Calendar', 'wooevents'), 
	...
	public function widget( $args, $instance ) {
		...
		wp_enqueue_style( 'ccw_tooltipster', CCW_PLUGIN_URL.'assets/css/widget/tooltipster.css' );
		wp_enqueue_style( 'ccw_tooltipster_shadow', CCW_PLUGIN_URL.'assets/css/widget/themes/tooltipster-shadow.css' );
		wp_enqueue_style( 'custom_css', CCW_PLUGIN_URL.'assets/css/widget/custom.css' );
		wp_enqueue_script( 'ccw_js', CCW_PLUGIN_URL.'assets/js/widget/jquery.tooltipster.min.js', array('jquery') );
			...
			foreach ( (array) $ak_post_titles as $ak_post_title ) {
					...
					if(is_array($event_data)) {
						...
						if($event_data != ''){
							...
							$date = date(get_option('time_format'), strtotime($time_arr[0]));
					...
					$post_title = __('Time:', 'wooevents').$date.' '.__('Event:','wooevents').$ak_post_title->post_title;//esc_attr( apply_filters( 'the_title', $ak_post_title->post_title, $ak_post_title->ID ) );
					$post_href=get_permalink($ak_post_title->ID);
					//echo "<pre>";print_r($post_title);echo "</pre>"; exit;
					if ( empty($ak_titles_for_day['day_'.$ak_post_title->dom]) )
						$ak_titles_for_day['day_'.$ak_post_title->dom] = '';
						$ak_titles_for_day['href_'.$ak_post_title->dom] = '';
					if ( empty($ak_titles_for_day["$ak_post_title->dom"]) ){ // first one
						$ak_titles_for_day["$ak_post_title->dom"] = $post_title;
						$ak_titles_for_day["href_$ak_post_title->dom"] = $post_href;
					}
					else{
	...
	//echo "<pre>";print_r($ak_titles_for_day);echo "</pre>"; //exit;
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( $day == gmdate('j', current_time('timestamp')) && $thismonth == gmdate('m', current_time('timestamp')) && $thisyear == gmdate('Y', current_time('timestamp')) )
				...
				if ( in_array($day, $daywithpost) ){ $class="event"; } else { $class=""; }
				$calendar_output .= '<td class="'.$class.'">';
			}
			if ( in_array($day, $daywithpost) ) // any posts today?
					...
					$calendar_output .= '<a style="cursor:pointer" href="' . esc_attr( $ak_titles_for_day[ 'href_'.$day ] ) .'" class="tooltip_interact" title="' . esc_attr( $ak_titles_for_day[ $day ] ) . "\">$day</a>";
			else
				$calendar_output .= $day;
			$calendar_output .= '</td>';
