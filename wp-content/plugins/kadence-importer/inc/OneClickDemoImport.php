  ...
  private $plugin_page_setup = array();
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			'page_title'  => esc_html__( 'One Click Demo Import' , 'kadence-importer' ),
			'menu_title'  => esc_html__( 'Import Demo Data' , 'kadence-importer' ),
			'capability'  => 'import',
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_enqueue_script( 'ocdi-main-js', PT_OCDI_URL . 'assets/js/main.js' , array( 'jquery', 'jquery-ui-dialog' ), KADENCE_IMPORTER_VERSION );
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'ajax_plugins_url' => self_admin_url( 'plugins.php' ),
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'kadence-importer' ),
						'dialog_title'          => esc_html__( 'Confirm Import', 'kadence-importer' ),
						'dialog_no'             => esc_html__( 'Cancel', 'kadence-importer' ),
						'dialog_yes_plugins'    => esc_html__( 'Install/activate plugins', 'kadence-importer' ),
						'dialog_yes'            => esc_html__( 'Import!', 'kadence-importer' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'kadence-importer' ),
					),
					'dialog_options' => apply_filters( 'pt-ocdi/confirmation_dialog_options', array( 'width' => '400' ) )
				)
			);

			wp_enqueue_style( 'ocdi-main-css', PT_OCDI_URL . 'assets/css/main.css', array() , KADENCE_IMPORTER_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'kadence-importer' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'kadence-importer' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'kadence-importer' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'kadence-importer' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdi/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks @ProteusThemes! #OCDI https://www.proteusthemes.com/', 'kadence-importer' );

				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sProteusThemes%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'kadence-importer' ),
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sKadence Importer%4$s plugin, because it has done its job.%5$s', 'kadence-importer' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'kadence-importer' ),
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'kadence-importer', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
