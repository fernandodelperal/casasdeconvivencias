<?php
/**
 * Plugin Name: WooCommerce Auto Restore Stock FDP
...
if (!class_exists('WC_Auto_Stock_Restore')) {
	...
	class WC_Auto_Stock_Restore {
		...
		public function __construct() {
			...
			add_action('woocommerce_order_status_cancelled_to_processing', array($this, 'restore_order_stock'), 10, 1);
			...
			foreach ($order->get_items() as $item) {
				...
				if ($item['product_id'] > 0) {
					...
					if ($_product && $_product->exists() && $_product->managing_stock()) {
						...
						$old_total_sales = get_post_meta($_product->id, 'total_sales', true);
						$qty = apply_filters('woocommerce_order_item_quantity', $item['qty'], $this, $item);

						// added this if clause to redecrease stock in case of going from canceled to processing FDP
						if ($order->get_status() == 'processing') {
							//$new_quantity = $_product->increase_stock(-$qty);
							$new_total_sales = $old_total_sales + 1;
						...
						update_post_meta($_product->id, 'total_sales', $new_total_sales);
						//A PARTIR DE UNA ACTUALIZACIÓN EMPEZÓ A RESTAURAR SIN NECESIDAD DEL PLUGIN, PERO DEJO LO DE ANTES PORQUE HAY QUE RESTAURAR Y DES-RESTAURAR EL METADEATA NEW_TOTAL_SALES
